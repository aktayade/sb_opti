...
namespace{
  struct OperationMigration : public FunctionPass {
    static char ID; // Pass identification, replacement for typeid
    double threshold;
    OperationMigration() : FunctionPass(ID) {}
	ProfileInfo* PI;
	LAMPLoadProfile * LP;
	DominatorTree *DT;
	Function *CurFunc;
				...
				if(trace_length > 3){
					traces.push_back(t2);
                                ...
                                if(word.compare("yes")==0){
                                        backedge.insert(make_pair(t1,true));
					if(trace_length > 3){
						backedge.insert(make_pair(t2,true));
                                	}
                                }
                                else{
                                        backedge.insert(make_pair(t1,false));
					if(trace_length > 3){
                                 	       backedge.insert(make_pair(t2,false));
                                	}
                                }
                        }
                }
...
//		errs()<<"unroll count = "<<unroll_count<<"\n";
		if(unroll_count == 0 )
			return;
...
//				errs()<<"qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqq\n";
		...
		t_clone.splice(t_clone.begin(),t);
		unroll(t_clone,unroll_count-1);
	}
...
char OperationMigration::ID = 0;
static RegisterPass<OperationMigration> X("op", "Operation Migration Pass");
