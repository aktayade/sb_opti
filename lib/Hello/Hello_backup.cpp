...
//		errs()<<"Finished\n";
	...
	void writeTracesToFile(list<trace> traces){
		ofstream os;
		os.open("superblocks",fstream::app);
		os<< "Function "<<(CurFunc->getName()).data()<<"\n";
		std::list<trace>::iterator itr;
		bool first;
		for(itr = traces.begin(); itr!=traces.end();itr++){
			first = false;
						...
						//++t1;
								    ...
								    if(find(temp.begin(), temp.end(), *t1) == temp.end() ){
                                      ...
                                      // We now have two operands...and we decide which superblock to merge with based on the execution count of the basic blocks
                                              ...
                                              float count1 = PI->getExecutionCount((BasicBlock*)inst->getOperand(1));
					      float count2 = PI->getExecutionCount((BasicBlock*)inst->getOperand(2));
