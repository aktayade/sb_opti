...
./$fname.slicm $2
opt -load /y/students/jezra/build/projects/project1/Debug+Asserts/lib/LLVMHello.so -lamp-inst-cnt -lamp-map-loop -lamp-load-profile -profile-loader -profile-info-file=llvmprof.out -op < $fname.slicm.bc > op.finish.bc || { echo "Failed to opt-load"; exit 1; }
