...
./$fname.slicm $2
opt -load /y/students/jezra/build/projects/project1/Debug+Asserts/lib/LLVMHello.so -lamp-inst-cnt -lamp-map-loop -lamp-load-profile -profile-loader -profile-info-file=llvmprof.out -op < $fname.slicm.bc > $fname.finish.bc || { echo "Failed to opt-load"; exit 1; }
opt -mem2reg < $fname.finish.bc > $fname.opmigration.bc
llc $fname.opmigration.bc -o $fname.opmigration.s
g++ $fname.opmigration.s -o $fname.opmigration
echo "After Operation migration"
./$fname.opmigration $2
